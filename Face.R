...
# 
# #test data  
# d.test <- read.csv("~/Dropbox/Kaggle/FaceProject/test.csv",stringsAsFactors=F);
# im.test <- foreach(im = d.test$Image, .combine=rbind) %dopar% {
#   as.integer(unlist(strsplit(im, " ")))
# }
# d.test$Image <- NULL

# save(d.train, im.train, d.test, im.test, file='~/Dropbox/Kaggle/FaceProject/data.Rd')

load('~/Dropbox/Kaggle/FaceProject/data.Rd')
